...
const navigationItems = [
  ...
  { name: 'Timesheets', href: '/accountant/timesheets', icon: Clock },
  { name: 'Expenses', href: '/accountant/expenses', icon: Receipt },
  { name: 'Invoices', href: '/accountant/invoices', icon: FileText },
  { name: 'Payroll', href: '/accountant/payroll', icon: DollarSign },
  { name: 'Learning', href: '/accountant/learning', icon: GraduationCap },
  { name: 'Goals', href: '/accountant/goals', icon: Target },
  { name: 'Reports', href: '/accountant/reports', icon: TrendingUp },
];

const quickActions = [
  { icon: Clock, label: 'Review Timesheets', color: 'text-blue-600', action: '/accountant/timesheets' },
  { icon: Receipt, label: 'Process Expenses', color: 'text-purple-600', action: '/accountant/expenses' },
  { icon: FileText, label: 'Create Invoice', color: 'text-green-600', action: '/accountant/invoices' },
  { icon: DollarSign, label: 'Process Payroll', color: 'text-orange-600', action: '/accountant/payroll' },
  { icon: Calculator, label: 'Financial Reports', color: 'text-indigo-600', action: '/accountant/reports' },
  { icon: TrendingUp, label: 'Analytics', color: 'text-cyan-600', action: 'coming-soon' },
];
    ...
    fetch('/api/auth/me')
      ...
      .then(data => {
        if (data.success && data.data) {
          ...
          if (!['ADMIN', 'ACCOUNTANT'].includes(data.data.role)) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Accountant Panel</p>
              </div>
            </div>
          </div>
      ...
      >
        ...
        <nav className="p-4 border-b border-slate-200">
          ...
          {navigationItems.map((item) => {
            ...
            const isActive = pathname === item.href || (item.href !== '/accountant/dashboard' && pathname.startsWith(item.href));
            return (
              <button
                key={item.href}
