 ...
 * Get rejected expense claims for admin's direct reports
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isRootLevelUser && directReportUserIds.length === 0) {
      ...
      // Regular admin with direct reports - show only direct reports' rejected expenses
      whereClause.userId = {
    ...
    console.error('[GET /api/admin/expenses/rejected] Error:', error);
    return NextResponse.json(
      {
        success: false,
