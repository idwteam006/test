/**
 * POST /api/admin/expenses/bulk-reject
 * Bulk reject multiple expense claims with a single reason (admin - direct reports only)
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const invalidExpenses = expenses.filter(
      (expense) => employeeManagerMap.get(expense.user.id) !== adminEmployee.id
    );
    ...
    console.error('[POST /api/admin/expenses/bulk-reject] Error:', error);
    return NextResponse.json(
      {
        success: false,
