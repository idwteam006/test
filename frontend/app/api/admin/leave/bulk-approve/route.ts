...
// POST /api/admin/leave/bulk-approve - Bulk approve leave requests (Admin)
export async function POST(request: NextRequest) {
  try {
    ...
    // Get employee record (manager/admin)
    ...
    // Verify authorization - managers and admins can only approve their direct reports' requests
    if ((user.role === 'MANAGER' || user.role === 'ADMIN') && employee) {
      for (const request of leaveRequests) {
        if (request.employee.managerId !== employee.id) {
          return NextResponse.json(
