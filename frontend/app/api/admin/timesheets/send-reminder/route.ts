/**
 * POST /api/admin/timesheets/send-reminder
 * Send a reminder to employees who haven't submitted their timesheets (admin - direct reports only)
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only admins can send reminders via this endpoint
    if (user.role !== 'ADMIN') {
      return NextResponse.json(
    ...
    // Get admin's employee ID to verify they manage these employees
    ...
    // Verify employees are direct reports
    if (adminEmployee) {
      const invalidEmployees = employees.filter(
        (emp) => emp.employee?.managerId !== adminEmployee.id
      );
      ...
      const emailContent = {
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <p>This is a friendly reminder from <strong>${adminName}</strong> to submit your timesheet${weekStart ? ` for the week of ${format(new Date(weekStart), 'MMMM d')} - ${format(new Date(weekEnd), 'MMMM d, yyyy')}` : ''}.</p>
    ...
    console.error('[POST /api/admin/timesheets/send-reminder] Error:', error);
    return NextResponse.json(
      {
        success: false,
