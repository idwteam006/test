...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      console.log(`[Expense Approved] Email sent to ${expense.user.email}`);
    } catch (emailError) {
      console.error('[Expense Approved] Failed to send email:', emailError);
    ...
    return NextResponse.json(
      { error: 'Failed to approve expense', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
