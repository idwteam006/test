 ...
 * Get pending expense approvals for manager's team
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (directReportUserIds.length === 0 && !isRootLevelUser) {
      // No direct reports and not root level, return empty result
      return NextResponse.json({
        success: true,
        expenses: [],
        summary: {
    ...
    // Note: We exclude the current user's own submissions since they can self-approve
    // via the auto-approval feature on their expenses page
    if (isRootLevelUser && directReportUserIds.length === 0) {
      // Root-level user with no direct reports - no pending approvals to show here
      // They can self-approve via the auto-approval feature
      return NextResponse.json({
        success: true,
        expenses: [],
        summary: {
      ...
      // Own submissions should be self-approved via auto-approval feature
      whereClause.userId = {
    ...
    console.error('[GET /api/manager/expenses/pending] Error:', error);
    return NextResponse.json(
      {
        success: false,
