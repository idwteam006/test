/**
 * GET /api/manager/timesheets/pending-submission
 * Get direct reports who have DRAFT timesheets or no entries (not yet submitted) - for sending reminders
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get all direct report employee details
    const allDirectReports = await prisma.user.findMany({
      where: {
        id: { in: directReportUserIds },
        tenantId,
      },
      select: {
        id: true,
    ...
    // Get ALL timesheet entries for all direct reports in the date range (any status)
    const allEntries = await prisma.timesheetEntry.findMany({
      where: {
        tenantId,
        userId: { in: directReportUserIds },
        workDate: {
          gte: weekStart,
          lte: weekEnd,
        },
    ...
    // Initialize all direct reports (even those with no entries)
    directReportUserIds.forEach(userId => {
    ...
    const notSubmittedEmployees = allDirectReports
      .filter(user => {
    ...
    const summary = {
      ...
      totalDirectReports: directReportUserIds.length,
    };
    ...
    console.error('[GET /api/manager/timesheets/pending-submission] Error:', error);
    return NextResponse.json(
      {
        success: false,
