/**
 * GET /api/manager/timesheets/rejected
 * Get rejected timesheet entries for manager's direct reports
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (isRootLevelUser && directReportUserIds.length === 0) {
      ...
      // Regular manager with direct reports - show only direct reports' rejected timesheets
      whereClause.userId = {
    ...
    console.error('[GET /api/manager/timesheets/rejected] Error:', error);
    return NextResponse.json(
      {
        success: false,
