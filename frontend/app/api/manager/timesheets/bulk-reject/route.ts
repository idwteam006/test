/**
 * POST /api/manager/timesheets/bulk-reject
 * Bulk reject multiple timesheet entries with a single reason
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Only managers and admins can bulk reject
    ...
    if (role === 'MANAGER') {
      ...
      const invalidEntries = entries.filter(
        (entry) => employeeManagerMap.get(entry.user.id) !== managerEmployee.id
      );
    ...
    console.error('[POST /api/manager/timesheets/bulk-reject] Error:', error);
    return NextResponse.json(
      {
        success: false,
