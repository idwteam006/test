 ...
 * Get pending timesheet approvals for manager's team
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Only managers, admins, and HR can access this endpoint
    if (!['MANAGER', 'ADMIN', 'HR'].includes(role)) {
      return NextResponse.json(
        { success: false, error: 'Unauthorized. Manager/HR access required.' },
        { status: 403 }
      );
    }
    ...
    // HR role can see all employees' timesheets in the tenant
    if (role === 'HR') {
      // HR can see all submitted timesheets except their own
      whereClause.userId = {
        not: userId,
      };
    } else {
      // For ADMIN and MANAGER, show only direct reports
    ...
    const teamSummary = pendingEntries.reduce((acc: any, entry) => {
      ...
      acc[userId].totalHours += entry.hoursWorked;
      if (entry.isBillable) {
        acc[userId].billableHours += entry.hoursWorked;
      }
    ...
    console.error('[GET /api/manager/timesheets/pending] Error:', error);
    return NextResponse.json(
      {
        success: false,
