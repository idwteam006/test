/**
 * GET /api/manager/timesheets/approved
 * Get approved timesheet entries for manager's team
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    // All roles see only their direct reports
    ...
    if (isRootLevelUser && directReportUserIds.length === 0) {
      ...
      // Regular manager with direct reports - show only direct reports' approved timesheets
      whereClause.userId = {
    ...
    console.error('[GET /api/manager/timesheets/approved] Error:', error);
    return NextResponse.json(
      {
        success: false,
