...
export default function EmployeeExpensesPage() {
  ...
  // Tab state for expense status filter
  const [statusTab, setStatusTab] = useState<'all' | 'pending' | 'approved' | 'rejected'>('all');

  // Overall stats (all-time, not filtered by week)
  ...
  const fetchOverallStats = async () => {
    try {
      ...
      if (data.success) {
        ...
        const pending = allExpenses.filter((e: Expense) => e.status === 'SUBMITTED');
        const approved = allExpenses.filter((e: Expense) => e.status === 'APPROVED' || e.status === 'PAID');
        const rejected = allExpenses.filter((e: Expense) => e.status === 'REJECTED');
        const drafts = allExpenses.filter((e: Expense) => e.status === 'DRAFT');

        setOverallStats({
          totalExpenses: allExpenses.length,
          totalAmount: allExpenses.reduce((sum: number, e: Expense) => sum + e.amount, 0),
          pendingCount: pending.length,
          pendingAmount: pending.reduce((sum: number, e: Expense) => sum + e.amount, 0),
          approvedCount: approved.length,
          approvedAmount: approved.reduce((sum: number, e: Expense) => sum + e.amount, 0),
          rejectedCount: rejected.length,
          rejectedAmount: rejected.reduce((sum: number, e: Expense) => sum + e.amount, 0),
          draftCount: drafts.length,
          draftAmount: drafts.reduce((sum: number, e: Expense) => sum + e.amount, 0),
        });
      }
    } catch (error) {
  ...
  // Filter expenses based on status tab
  const filteredExpenses = useMemo(() => {
    if (statusTab === 'all') return expenses;
    if (statusTab === 'pending') return expenses.filter(e => e.status === 'SUBMITTED' || e.status === 'DRAFT');
    if (statusTab === 'approved') return expenses.filter(e => e.status === 'APPROVED' || e.status === 'PAID');
    if (statusTab === 'rejected') return expenses.filter(e => e.status === 'REJECTED');
    return expenses;
  }, [expenses, statusTab]);
  ...
  const handleSubmitExpense = async (expenseId: string) => {
    ...
    try {
      const response = await fetch(`/api/employee/expenses/${expenseId}/submit`, {
        method: 'POST',
      });
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <Link
          href="/hr/expenses/dashboard"
          className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:from-purple-700 hover:to-indigo-700 transition-all shadow-md w-fit"
        >
      ...
      {/* Overall Stats Cards (All-Time) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        {/* Total Expenses Card */}
        <div className="group relative overflow-hidden rounded-2xl bg-white/70 backdrop-blur-xl border border-white/50 shadow-xl shadow-slate-200/50 p-5 transition-all duration-300 hover:shadow-2xl hover:shadow-purple-200/30 hover:-translate-y-1">
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-3">
              ...
              <Badge className="bg-purple-100 text-purple-700 border-0 font-medium text-xs">All Time</Badge>
            </div>
            <div className="space-y-1">
              <p className="text-3xl font-bold text-slate-900">{overallStats.totalExpenses}</p>
              <p className="text-sm text-slate-500 font-medium">Total Expenses</p>
              <p className="text-xs text-purple-600">{formatCurrency(overallStats.totalAmount)}</p>
            </div>
          </div>
        </div>
        ...
        {/* Pending Card */}
        <div
          className="group relative overflow-hidden rounded-2xl bg-white/70 backdrop-blur-xl border border-white/50 shadow-xl shadow-slate-200/50 p-5 transition-all duration-300 hover:shadow-2xl hover:shadow-amber-200/30 hover:-translate-y-1 cursor-pointer"
          onClick={() => setStatusTab('pending')}
        >
          <div className="absolute inset-0 bg-gradient-to-br from-amber-500/5 to-orange-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          <div className={`absolute inset-0 ${statusTab === 'pending' ? 'ring-2 ring-amber-500 rounded-2xl' : ''}`} />
          <div className="relative">
            <div className="flex items-center justify-between mb-3">
              <div className="p-2 rounded-xl bg-gradient-to-br from-amber-100 to-orange-100">
                <Clock className="h-5 w-5 text-amber-600" />
              </div>
              <Badge className="bg-amber-100 text-amber-700 border-0 font-medium text-xs">Pending</Badge>
            </div>
            <div className="space-y-1">
              <p className="text-3xl font-bold text-slate-900">{overallStats.pendingCount}</p>
              <p className="text-sm text-slate-500 font-medium">Awaiting Review</p>
              <p className="text-xs text-amber-600">{formatCurrency(overallStats.pendingAmount)}</p>
            </div>
          </div>
        </div>
        ...
        {/* Approved Card */}
        <div
          className="group relative overflow-hidden rounded-2xl bg-white/70 backdrop-blur-xl border border-white/50 shadow-xl shadow-slate-200/50 p-5 transition-all duration-300 hover:shadow-2xl hover:shadow-emerald-200/30 hover:-translate-y-1 cursor-pointer"
        ...
        >
          ...
          <div className={`absolute inset-0 ${statusTab === 'approved' ? 'ring-2 ring-emerald-500 rounded-2xl' : ''}`} />
          <div className="relative">
            <div className="flex items-center justify-between mb-3">
              <div className="p-2 rounded-xl bg-gradient-to-br from-emerald-100 to-green-100">
                <CheckCircle className="h-5 w-5 text-emerald-600" />
              </div>
              <Badge className="bg-emerald-100 text-emerald-700 border-0 font-medium text-xs">Approved</Badge>
            </div>
            <div className="space-y-1">
              <p className="text-3xl font-bold text-slate-900">{overallStats.approvedCount}</p>
              <p className="text-sm text-slate-500 font-medium">Total Approved</p>
              <p className="text-xs text-emerald-600">{formatCurrency(overallStats.approvedAmount)}</p>
            </div>
          </div>
        </div>
        ...
        {/* Rejected Card */}
        <div
          className="group relative overflow-hidden rounded-2xl bg-white/70 backdrop-blur-xl border border-white/50 shadow-xl shadow-slate-200/50 p-5 transition-all duration-300 hover:shadow-2xl hover:shadow-red-200/30 hover:-translate-y-1 cursor-pointer"
        ...
        >
          ...
          <div className={`absolute inset-0 ${statusTab === 'rejected' ? 'ring-2 ring-red-500 rounded-2xl' : ''}`} />
          <div className="relative">
            <div className="flex items-center justify-between mb-3">
              <div className="p-2 rounded-xl bg-gradient-to-br from-red-100 to-rose-100">
                <XCircle className="h-5 w-5 text-red-600" />
              </div>
              <Badge className="bg-red-100 text-red-700 border-0 font-medium text-xs">Rejected</Badge>
            </div>
            <div className="space-y-1">
              <p className="text-3xl font-bold text-slate-900">{overallStats.rejectedCount}</p>
              <p className="text-sm text-slate-500 font-medium">Total Rejected</p>
              <p className="text-xs text-red-600">{formatCurrency(overallStats.rejectedAmount)}</p>
            </div>
          </div>
        </div>
        ...
        {/* Drafts Card */}
        <div
          className="group relative overflow-hidden rounded-2xl bg-white/70 backdrop-blur-xl border border-white/50 shadow-xl shadow-slate-200/50 p-5 transition-all duration-300 hover:shadow-2xl hover:shadow-slate-200/30 hover:-translate-y-1 cursor-pointer"
          onClick={() => setStatusTab('pending')}
        >
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-3">
              ...
              <Badge className="bg-slate-100 text-slate-700 border-0 font-medium text-xs">Drafts</Badge>
            </div>
            <div className="space-y-1">
              <p className="text-3xl font-bold text-slate-900">{overallStats.draftCount}</p>
              <p className="text-sm text-slate-500 font-medium">Not Submitted</p>
              <p className="text-xs text-slate-600">{formatCurrency(overallStats.draftAmount)}</p>
            </div>
          </div>
        </div>

        {/* Total Amount Card */}
        <div className="group relative overflow-hidden rounded-2xl bg-white/70 backdrop-blur-xl border border-white/50 shadow-xl shadow-slate-200/50 p-5 transition-all duration-300 hover:shadow-2xl hover:shadow-blue-200/30 hover:-translate-y-1">
          ...
          <div className="relative">
            ...
            <div className="space-y-1">
              <p className="text-2xl font-bold text-slate-900">{formatCurrency(overallStats.approvedAmount + overallStats.pendingAmount)}</p>
              <p className="text-sm text-slate-500 font-medium">Active Amount</p>
              <p className="text-xs text-blue-600">Pending + Approved</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Status Tab Filter */}
      <div className="flex items-center gap-2 bg-white/70 backdrop-blur-xl border border-white/50 rounded-2xl p-2 shadow-lg w-fit">
        <button
        ...
        >
          <div className="flex items-center gap-2">
            ...
            Pending ({stats.pending + expenses.filter(e => e.status === 'DRAFT').length})
          </div>
        </button>
        <button
          onClick={() => setStatusTab('approved')}
